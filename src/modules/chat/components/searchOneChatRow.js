...
import { getChatLogo, getNofiy } from "utils";

function SearchOneChatRow({ joinChat, chat, myId }) {
  const { onlineUsers, notifications } = useModal();
  return (
    <div className="message-row" onClick={() => joinChat(get(chat, "_id"))}>
      <div
      ...
      <div className="message-user-data">
        ...
        <div className="user-data-bottom">
          <span className="message-user-last-message">
            {chat.type === "group"
              ? size(chat.users) + " ta a'zo"
              : "yaqinda tarmoqda edi"}
          </span>
          {getNofiy(notifications, chat) > 0 && (
...
export default SearchOneChatRow;
